...
function recupLastInstruction(e) {
    ...
    else if (e.keyCode == '37' && instruction !== "droite") instruction = "gauche";
    else if (e.keyCode == '39' && instruction !== "gauche") instruction = "droite";
}
...
function deplacement() {
    ...
    switch (instruction) {
        case 'haut':    nouvelleTete = ancienneTete <= taille ? ancienneTete + taille * (taille - 1) : ancienneTete - taille; break;
        case 'bas':     nouvelleTete = ancienneTete > taille * (taille - 1) ? ancienneTete - taille * (taille - 1) : ancienneTete + taille; break;
        case 'gauche':  nouvelleTete = ancienneTete % taille === 1 ? ancienneTete + (taille - 1) : ancienneTete - 1; break;
        case 'droite':  nouvelleTete = ancienneTete % taille === 0 ? ancienneTete - (taille - 1) : ancienneTete + 1; break;
    }
...
function changeDirection(dir) {
    const opposites = { haut: "bas", bas: "haut", gauche: "droite", droite: "gauche" };
    if (instruction !== opposites[dir]) {
        instruction = dir;
    }
}

// Envoie le score vers le backend PHP
